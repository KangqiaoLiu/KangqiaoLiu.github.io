---
layout: archive
title: "PUBLICATIONS"
permalink: /publications/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

You can also find my articles on my [Google Scholar](https://scholar.google.com/citations?user=utIJkHcAAAAJ&hl=en) profile.<br />
The symbol \* represents the equal contribution.

## **Preprints**
1. **Kangqiao Liu**<br />
  *Classical codes violate the conjectured square-root bound for quantum random access codes*<br />
  [[arXiv:2607.15617]](https://arxiv.org/abs/2607.15617) (2026.07.20). <a href="{{ '/publications/qrac_counterexample.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="qrac" lang="en" %}
2. Deyou Chen, Chuang Yang, and **Kangqiao Liu**<br />
  *Chaos bound for spinning particles in Kerr-Newman-AdS black holes*<br />
  [[arXiv:2607.00432]](https://arxiv.org/abs/2607.00432) (2026.07.01). <a href="{{ '/publications/2607.00432v2.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="gravity_ads" lang="en" %}
3. Jie Gu and **Kangqiao Liu**<br />
  *Finite-frequency fluctuation-response bounds for open quantum systems*<br />
  [[arXiv:2605.03340]](https://arxiv.org/abs/2605.03340) (2026.05.06). <a href="{{ '/publications/2605.03340v1.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="open_frequency" lang="en" %}

## **Peer-reviewed**
1. **Kangqiao Liu** and Deyou Chen<br />
  *Maximal-velocity deficit under a finite-support constraint in hard-wall half-line continuous-time quantum walk*<br />
  [To be published in Physical Review A](https://journals.aps.org/pra/accepted/10.1103/dyyr-z1k8) (2026.09.01). <a href="{{ '/publications/AQ12647.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="hardwall_ctqw" lang="en" %}
2. Chuang Yang, Chuanhong Gao, Deyou Chen, and **Kangqiao Liu**<br />
  *Bound on Lyapunov exponents with spinning particles in Kerr–Newman spacetimes*<br />
  [The European Physical Journal C **86**, 677 (2026)](https://link.springer.com/article/10.1140/epjc/s10052-026-15894-8) (2026.06.22) <a href="{{ '/publications/s10052-026-15894-8.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="gravity_kn" lang="en" %}
3. **Kangqiao Liu** and Jie Gu<br />
  *Response kinetic uncertainty relation for Markovian open quantum systems*<br />
  [Physical Review A **113**, 062443 (2026)](https://journals.aps.org/pra/abstract/10.1103/ps1b-8l1x) (2026.06.16) <br />
  [press release](https://lxy.xhu.edu.cn/b3/5a/c575a242522/page.htm) (in Chinese) by Department<br />
  [[arXiv:2501.04895]](https://arxiv.org/abs/2501.04895). <a href="{{ '/publications/ps1b-8l1x.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="open_rkur" lang="en" %}
4. **Kangqiao Liu**, Masaya Nakagawa, and Masahito Ueda<br />
  *Maxwell's demon for quantum transport*<br />
  [Physical Review A **113**, 022436 (2026)](https://journals.aps.org/pra/abstract/10.1103/rhvv-yd6w) (2026.02.23) <br />
  [press release](https://lxy.xhu.edu.cn/b3/66/c575a242534/page.htm) (in Chinese) by Department<br />
  [[arXiv:2303.08326]](https://arxiv.org/abs/2303.08326). <a href="{{ '/publications/PRA_113_022436_2026_Published_Article.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="quantum_transport" lang="en" %}
5. **Kangqiao Liu** and Jie Gu<br />
  *Dynamical activity universally bounds precision of response in Markovian nonequilibrium systems*<br />
  [Communications Physics **8**, 62 (2025)](https://www.nature.com/articles/s42005-025-01982-w) (2025.02.11)<br />
  see [*Behind the Paper*](https://go.nature.com/3WYFOSp) for a blog post<br />
  [press release](https://lxy.xhu.edu.cn/63/95/c575a222101/page.htm) (in Chinese) by Department<br />
  [[arXiv:2410.20800]](https://arxiv.org/abs/2410.20800). <a href="{{ '/publications/s42005-025-01982-w.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="stoch_response" lang="en" %}
6. Jiale Yu\*, Shiyu Wang\*, **Kangqiao Liu**, Chen Zha, Yulin Wu, Fusheng Chen, Yangsen Ye, Shaowei Li, Qingling Zhu, Shaojun Guo, Haoran Qian, He-Liang Huang, Youwei Zhao, Chong Ying, Daojin Fan, Dachao Wu, Hong Su, Hui Deng, Hao Rong, Kaili Zhang, Sirui Cao, Jin Lin, Yu Xu, Cheng Guo, Na Li, Futian Liang, Yong-Heng Huo, Chao-Yang Lu, Cheng-Zhi Peng, Kae Nemoto, W. J. Munro, Xiaobo Zhu, Jian-Wei Pan, and Ming Gong<br />
  *Experimental demonstration of a Maxwell's demon quantum battery in a superconducting NISQ processor*<br />
  [Physical Review A **109**, 062614 (2024)](https://journals.aps.org/pra/abstract/10.1103/PhysRevA.109.062614) (2024.06.20). <a href="{{ '/publications/PhysRevA.109.062614.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="quantum_battery" lang="en" %}
7. Takashi Mori, Liu Ziyin, **Kangqiao Liu**, and Masahito Ueda<br />
  *Power-law escape rate of SGD*<br />
  [Proceedings of the 39th International Conference on Machine Learning, PMLR **162**:15959-15975, 2022 (ICML 2022)](https://proceedings.mlr.press/v162/mori22a.html) (2022.07.15)<br />
  selected for **Spotlight** <br />
  [[arXiv:2105.09557]](https://arxiv.org/abs/2105.09557). <a href="{{ '/publications/mori22a.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="ml_escape" lang="en" %}
8. Liu Ziyin\*, **Kangqiao Liu**\*, Takashi Mori, and Masahito Ueda<br />
  *Strength of Minibatch Noise in SGD*<br />
  [The 10th International Conference on Learning Representations (ICLR 2022)](https://openreview.net/forum?id=uorVGbWV5sw) (2022.01.29)<br />
  selected for **Spotlight** (5% of all submissions)<br />
  [[arXiv:2102.05375]](https://arxiv.org/abs/2102.05375). <a href="{{ '/publications/4355_strength_of_minibatch_noise_in.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a>{% include publication-feature.html id="ml_minibatch" lang="en" %}
9. **Kangqiao Liu**\*, Liu Ziyin\*, and Masahito Ueda<br />
  *Noise and Fluctuation of Finite Learning Rate Stochastic Gradient Descent*<br />
  [Proceedings of the 38th International Conference on Machine Learning, PMLR **139**:7045-7056, 2021 (ICML 2021)](http://proceedings.mlr.press/v139/liu21ad.html) (2021.07.01)<br />
  [[arXiv:2012.03636]](https://arxiv.org/abs/2012.03636). <a href="{{ '/publications/liu21ad.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a> <a href="{{ '/publications/liu21ad-supp.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">Supplements</a>{% include publication-feature.html id="ml_finite_lr" lang="en" %}
10. **Kangqiao Liu**, Zongping Gong, and Masahito Ueda<br />
  *Thermodynamic Uncertainty Relation for Arbitrary Initial States*<br />
  [Physical Review Letters **125**, 140602 (2020)](https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.125.140602) (2020.09.29)<br />
  [[arXiv:1912.11797]](https://arxiv.org/abs/1912.11797). <a href="{{ '/publications/PhysRevLett.125.140602.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">PDF</a> <a href="{{ '/publications/kqliu_SM_v3.pdf' | relative_url }}" class="btn btn--small" target="_blank" rel="noopener">Supplements</a>{% include publication-feature.html id="stoch_tur" lang="en" %}




<!-- {% if author.googlescholar %} -->
  
<!-- {% endif %} -->

<!-- {% include base_path %}

{% for post in site.publications reversed %}
  {% include archive-single.html %}
{% endfor %} -->
